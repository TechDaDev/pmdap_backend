...
@deconstructible
class PrivateIdentityStorage(_StorageBase):
    """Private identity file storage.

    Local backend stores under ``IDENTITY_FILE_ROOT``. S3 backend stores under
    the ``identity`` key prefix in the configured bucket. Both backends keep
