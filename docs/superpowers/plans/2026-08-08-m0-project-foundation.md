...
- [x] **Step 1: Pin M0 runtime dependencies**
...
- [x] **Step 2: Pin test and quality dependencies**

Add pytest, pytest-django, pytest-cov, factory_boy, Faker, and Ruff.

- [x] **Step 3: Configure test and quality commands**
...
- [x] **Step 1: Write app-boundary tests**
...
- [x] **Step 2: Write model foundation tests**
...
- [x] **Step 3: Write health and OpenAPI contract tests**
...
- [x] **Step 4: Write factory foundation**
...
- [x] **Step 6: Commit failing contracts**
...
- [x] **Step 1: Create settings split**
...
- [x] **Step 5: Verify GREEN for foundation tests**

Run: `.venv/bin/pytest tests/test_app_structure.py tests/test_models.py -q --no-cov`

Expected: all selected tests pass.

- [x] **Step 6: Run Django checks**
...
- [x] **Step 1: Verify endpoint tests remain RED**

Run: `.venv/bin/pytest tests/test_health.py tests/test_openapi.py -q --no-cov`

Expected: 404 failures for missing routes.

- [x] **Step 2: Implement liveness endpoint**

Create `HealthView` with `AllowAny`, no authentication classes, documented
`{"status": "ok"}` response, and no dependency details.

- [x] **Step 3: Add versioned routes**
...
- [x] **Step 5: Review diff and scope**
...
- Local tests: 15 passed, 0 failed
- Branch-aware coverage: 100%
- Ruff lint/format: passed
- Django system, deploy, and migration drift checks: passed
- Docker Compose build and startup: passed
- PostgreSQL migration check: passed
- Redis: `PONG`
- Celery: one worker returned `pong`
- Health: HTTP 200, `{"status":"ok"}`
- OpenAPI: HTTP 200 and health contract present
- Runtime logs: no `ERROR` or `Traceback`
